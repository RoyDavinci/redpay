...
export const Transactions = () => {
  ...
  const debouncedSetFilterValue = debounce((value) => {
    ...
    if (value && !/^(?:\+234|0)?[789]\d{0,9}$/.test(value)) {
  ...
  const filterTable = async (e) => {
    ...
    try {
      ...
      const { data: portingData } = await axios.get(
        `http://localhost:8000/network.php?phone=${encodeURIComponent(formattedValue)}`
      );
      ...
      const { data } = await axios.get(
        `http://localhost:8000/number.php?phone=${encodeURIComponent(formattedValue)}&partial=${isPartial}`
      );
  ...
  const handleTimestamp = (e) => {
    ...
    // Just trigger a new API call with current timeStamp values
    getItems();
  };
  ...
  const getItems = async () => {
    ...
    try {
         ...
         const query = `http://localhost:8000/messages.php?page=${page}&pageSize=50&from=${encodeURIComponent(fromDate)}&to=${encodeURIComponent(toDate)}&status=${encodeURIComponent(status)}&telco=${encodeURIComponent(telco)}`;
                ...
                >
                  <option value='select'>Select Status</option>
                   <option value='DELIVRD'>DELIVERED</option>
                    ...
                    <option value=''>PENDING</option> {/* Empty for null/empty values */}
